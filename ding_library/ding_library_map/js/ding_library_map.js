...
(function($) {
    ...
    Drupal.DingLibraryMapController = function(mapId, options) {
        ...
        self.init = function() {
            ...
            $('<a class="resize expand" href="#">Expand map</a>').appendTo(self.mapContainer).toggle(function(event) {
                self.expandMap();
                return false;
            }, function(event) {
                self.resizeMap(200);
            ...
            if (initLat && initLon) {
                ...
                self.expandMap();
                $.scrollTo(self.mapContainer, '500', {
                    offset: -20
                });
            }

            // Set up the info bubble that appears when hovering a marker.
            self.infoBox = $('<div id="library-info" style="display: none;">' + '<div class="frame">' + '<div class="inner">' + '<h3 class="name"></h3>' + '<div class="address">' + '<div class="street"></div>' + '<div><span class="postal-code"></span> <span class="city"></span></div>' + '</div>' + '<dl class="opening-hours"></dl>' + '</div>' + '</div>' + '</div>');
        ...
        self.updateInfoBox = function(marker, markerData) {
            ...
            while (day < days.length) {
                ...
                if (markerData.opening_hours[[days[day]]] && markerData.opening_hours[[days[day]]].length > 0) {
                    ...
                    while ((nextDay < days.length) && (markerData.opening_hours[[days[nextDay]]].length > 0) && (startTime !== null) && (endTime !== null) && (startTime == markerData.opening_hours[[days[nextDay]]][0].open) && (endTime == markerData.opening_hours[[days[nextDay]]][0].close)) {
            ...
            self.infoBox.css({
                'left': (point.x - 10) + 'px',
                'top': (point.y - self.infoBox.outerHeight()) + 4 + 'px'
            }).show();
        };
    ...
    Drupal.behaviors.openlayersDingLibraryMap = function(context) {
        ...
        if (Drupal.settings.dingLibraryMap.initialised) {
            return;
        }
